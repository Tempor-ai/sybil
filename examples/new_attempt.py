...
if uploaded_file is not None:
    ...
    # Assuming generate_next_dates is defined elsewhere and works correctly
    ...
    if future_file is not None:
        ...
        future_time_col_index = st.selectbox("Select Future Time Column Index", range(len(future_dataset.columns)),
                                             format_func=lambda x: future_dataset.columns[x])
        future_time_col = future_dataset.columns[future_time_col_index]

        # Ensure the future time column is in datetime format
        ...
        # Debug: Check the future test data before sending to API
        st.write("Future Test Data:")
        st.write(future_test_data)

        with st.spinner("Forecasting future data..."):
            api_json = {
                'model': model,
                'data': future_test_data[future_time_col].astype(str).tolist()
            }

            # Debug: Print the exact data being sent to the API
            st.write("Data being sent to the API:")
            st.json(api_json)
            ...
            if response.status_code == 200:
                try:
                    ...
                    if 'data' in future_forecast_json_out:
                        future_forecast_df = pd.DataFrame(
                            data=future_forecast_json_out['data'],
                            columns=[future_time_col, target_col],
                        )
                        ...
                        future_forecast_df.set_index(future_time_col)[target_col].plot(ax=ax, color='green',
                                                                                       label='Future Forecast')
                        ax.legend()
                        st.pyplot(fig)
                    else:
                        st.error("The API response does not contain the expected 'data' field.")
